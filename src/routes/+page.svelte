...
<div class="flex justify-center">	
	<div class="grid grid-cols-1 gap-4 min-w-full md:min-w-[750px]">
		...
		<div class="card p-4 w-full text-token space-y-4">
			<label class="label"><span>Snippet Title</span>
			<input class="input" type="text" placeholder="Your snippet title.." bind:value={formData.title}/></label>
			<label class="label">
			...
			<label class="label"><span>Code Snippet</span>
				<textarea class="textarea" rows="4" placeholder="Your code snippet.." bind:value={formData.code}/></label>
		...
		{:else} <span class="text-center">Empty for now, start saving snippets!</span>
		{/each}
	</div>
</div>